...
function Header({ dataUser }: { dataUser: randomUser[] }) {
  const [friendsOpen, toggleFriends] = useToggle(false);
  const [burgerOpen, toggleBurger] = useToggle(false);
      ...
      <div className="sidebar__scroll">
        ...
        <section className="profil">
          ...
          <div className="profil__friends">
            <button type="button" className="button--disable" onClick={() => toggleFriends()}>
              <i className="fa-solid fa-user-group" />
            </button>
            <MenuFriends dataUser={dataUser} friendsOpen={friendsOpen} toggleFriends={toggleFriends} />
          </div>
          <div className="profil__burger">
            <button type="button" className="button--disable button__burger" onClick={() => toggleBurger()}>
        ...
        <div className="friends">
          ...
          <div className="friends__data">
            {dataUser?.map((data, i) => {
              return (
                i !== 0 && (
                  <div key={dataUser[0]?.login.uuid + i} className="friends__global">
                    <img className="friends__img" src={data.picture.large} alt={`picture of ${data?.name.first}`} />
